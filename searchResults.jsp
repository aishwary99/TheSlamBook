...
<!-- Mirrored from themedesigner.in/demo/adminbite/html/light-sidebar/starter-kit.html by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 11 Oct 2019 18:26:52 GMT -->
<head>
    <meta charset="utf-8">
                        ...
                        <li>
                            ...
                            <div class="user-profile dropdown m-t-20">
                                ...
                                <div class="user-content hide-menu m-t-10">
                                    ...
                                    <a href="profile.jsp" class="btn btn-circle btn-sm m-r-5" id="Userdd" role="button" data-toggle="dropdown" aria-haspopup="true"
                                        aria-expanded="false">
        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                    ...
                    <div class="table-responsive m-t-20">
                            <%
                            List<Member> searchResults=(List<Member>)session.getAttribute("searchResults");
                            MemberDataStructure memberDataStructure=(MemberDataStructure)session.getAttribute("mds");
                            List<Member> requestsSent=memberDataStructure.getRequestsSent();
                            List<Member> requestsArrived=memberDataStructure.getRequestsArrived();
                            MemberBean member=(MemberBean)session.getAttribute("memberBean");
                            int memberCode=member.getCode();
                            for(Member m:searchResults)
                            {
                                if(m.getCode()==memberCode)
                                {
                            %>
                            ...
                            <table class="table table-responsive-sm">
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="padding-top:30px;">
                                            <p style="width:150px;" class="btn btn-default">Friends</p>
                                        </th>
                                    </tr>
                                </tbody>
                            </table>
                            <%
                                ...
                                for(Member requestSender:requestsSent)
                                {
                                    alloted=false;         
                                    if(m.getCode()==requestSender.getCode())
                                    {    
                            %>
                            ...
                            <table class="table table-responsive-sm">
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="padding-top:30px;">
                                            <span type="text" class="btn btn-primary"  name="addFriendRequest<%=m.getCode()%>">Friend Request Sent</span>
                                            <span type="button" class="btn btn-danger" id="cancelRequestButton<%=m.getCode()%>" name="cancelRequestButton<%=m.getCode()%>" onclick="cancelFriendRequest(<%=m.getCode()%>)"><span class="ti-close"></span></span>
                                            <input type="hidden" id="memberName" name="memberName" value="<%=m.getName()%>">
                                        </th>
                                    </tr>
                                </tbody>
                            </table>
                            <%
                                ...
                                if(!alloted)
                                {                                
                                    for(Member requestReceiver:requestsArrived)
                                    {
                                        if(m.getCode()==requestReceiver.getCode())
                                        {    
                            %>
                            ...
                            <table class="table table-responsive-sm">
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="padding-top:30px;">
                                            <span type="button" class="btn btn-primary" onclick="acceptFriendRequest()" id="acceptFriendRequest<%=m.getCode()%>" name="acceptFriendRequest<%=m.getCode()%>">Accept</span>
                                            <span type="button" class="btn btn-danger" id="cancelRequestButton<%=m.getCode()%>" name="cancelRequestButton<%=m.getCode()%>" onclick="cancelFriendRequest(<%=m.getCode()%>)"><span class="ti-close"></span></span>
                                            <input type="hidden" id="requestSenderName" name="requestSenderName" value="<%=m.getName()%>">
                                            <input type="hidden" id="requestSenderCode" name="requestSenderCode" value="<%=m.getCode()%>">
                                        </th>
                                    </tr>
                                </tbody>
                            </table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="padding-top:30px;">
                                            ...
                                            <span type="button" class="btn btn-primary" onclick="addFriendRequest(<%=m.getCode()%>)" id="addFriendRequest<%=m.getCode()%>" name="addFriendRequest<%=m.getCode()%>">Add Friend</span>
                                            <span type="button" class="btn btn-danger hide" id="cancelRequestButton<%=m.getCode()%>" name="cancelRequestButton<%=m.getCode()%>" onclick="cancelFriendRequest(<%=m.getCode()%>)"><span class="ti-close"></span></span>
                                            <input type="hidden" id="memberName" name="memberName" value="<%=m.getName()%>">
                                        </th>
                                    </tr>
                                </tbody>
